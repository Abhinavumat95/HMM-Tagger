...
def pair_counts(sequences_A, sequences_B):
    ...
    dct = dict(Counter(zip(sequences_A, sequences_B)))
    Dict = {}
    for key,value in dct.items():
        if key[0] not in Dict.keys():
            Dict[key[0]] = {k[1]:v for k,v in dct.items() if k[0] == key[0]}
    return (Dict)
...
def Word_Counts(seq_A, seq_B):
    dct = dict(Counter(list(zip(seq_A, seq_B))))
...
word_counts = Word_Counts(words, tags)
mfc_table = {}
...
def bigram_counts(sequences):
    ...
    # TODO: Finish this function!
    Dict = {}
    S_1, S_2 = itertools.tee(sequences)
    next(S_2, None)
    Z = zip(S_1, S_2)
    Dict = Counter(Z)
    return Dict
...
for tag in emission_counts.keys():
    ...
    for word in emission_counts[tag]:
        Dict_1[word] = emission_counts[tag][word]/tag_count
    emission_p = DiscreteDistribution(Dict_1)
...
for tag in tag_starts:
    basic_model.add_transition(basic_model.start, Dict[tag], tag_starts[tag]/len(data.training_set.Y))
    basic_model.add_transition(Dict[tag], basic_model.end, tag_ends[tag]/tag_unigrams[tag])
    
for (tag1, tag2) in tag_bigrams:
    basic_model.add_transition(Dict[tag1], Dict[tag2], tag_bigrams[(tag1,tag2)]/tag_unigrams[tag1])
